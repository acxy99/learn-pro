    ...
    <div class="container pt-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent p-0 mb-4">
                ...
                <li  class="breadcrumb-item d-inline-flex align-self-center" aria-current="page">PLA</li>
            </ol>
        </nav>
        ...
        <div class="bg-light p-3 mb-5">
            ...
            <div style="overflow-x:auto">
                <table class="bg-white table table-hover table-bordered mb-3">
                    ...
                    <tbody>
                        <tr v-for="p in pla" :key="p.id" @mouseover="active = p.id" @mouseout="active = ''" style="height: 75px">
                            <td style="width: 5%">{{p.id}}</td>
                            <td style="width: 50%">{{p.question}}</td>
                            <td style="width: 10%">{{p.qtype}}</td>
                            <td style="width: 10%">{{p.status}}</td>
                            <td style="width: 10%">{{p.mark}}</td>
                            <td style="width: 15%">
                                <div v-show="active == p.id">
                                    <a class="btn p-1" :href="getEditPlaUrl(p)">
                                        <i class="material-icons">create</i>
                                    </a>
                                    <button class="btn p-1" style="background-color: transparent" @click="deletePla(p)" >
                ...
                <li :class="[{disabled: !pagination.prev_page_url}]" class="page-item"><a class="page-link" href="#" @click="getPla(pagination.prev_page_url)">Previous</a></li>
                <li class="page-item disabled"><a class="page-link text-dark" href="#">Page {{ pagination.current_page }} of {{ pagination.last_page }}</a></li>
                <li :class="[{disabled: !pagination.next_page_url}]" class="page-item"><a class="page-link" href="#" @click="getPla(pagination.next_page_url)">Next</a></li>
            </ul>
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            createQuestionUrl:'/admin/courses/'+this.course.slug +'/topic/'+this.topic.id+'/pla/create',
            pla:[],
            pagination: {},
        };
    },
    ...
    methods: {
        getPla(url) {
            url = url || '/api/admin/courses/topic/'+this.topic.id+'/pla'
        ...
        getEditPlaUrl(pla) {
            return '/admin/courses/' + this.course.slug + '/topic/'+this.topic.id+'/pla/' + pla.id + '/edit';
        },
        deletePla(pla) {
            if(confirm('Are you sure you want to delete this question?')) {
                axios.delete('/admin/pla/' + pla.id)
                    .then(response => {
                        this.getPla();
                    })
                    .catch(error => {
